...
bool is_avl_tree2(avl_node_t *root);

void bag_insert_norot(bag_t *bag, bag_elem_t elem);

void avl_insert_norot(avl_node_t **head, bag_elem_t elem, int (*cmp)(bag_elem_t, bag_elem_t));
...
bool is_avl_tree2(avl_node_t *root) {
    ...
    if (root->left == NULL && HEIGHT(root->right) == 1) {
        return true;
    } else if (root->right == NULL && HEIGHT(root->left) == 1) {
        return true;
    } else if (root->left == NULL && root->right == NULL) {
        return true;
    } else {
        return is_avl_tree2(root->left) && is_avl_tree2(root->right);
    }
}

bool is_avl_tree(bag_t *bag) {
    return is_avl_tree2(bag->root);
}

void bag_insert_norot(bag_t *bag, bag_elem_t elem) {
    (bag->size)++;
    avl_node_t **head = &(bag->root);
    avl_insert_norot(head, elem, bag->cmp);
}

void avl_insert_norot(avl_node_t **head, bag_elem_t elem, int (*cmp)(bag_elem_t, bag_elem_t)) {
    if (!*head) {
        avl_node_t *new = avl_node_create(elem);
        *head = new;
        avl_update_height(*head);
    }
    if (cmp(elem, (*head)->elem)) {
        if ((*head)->right == NULL) {
            avl_node_t *new = avl_node_create(elem);
            (*head)->right = new;
            avl_update_height(*head);
            ...
            avl_insert_norot(&((*head)->right), elem, (*cmp));
            avl_update_height(*head);
            return;
        }
    } else {
        if ((*head)->left == NULL) {
            ...
            avl_insert_norot(&((*head)->left), elem, (*cmp));
            avl_update_height(*head);
            return;
        }
...
bool avl_remove2(avl_node_t **root, bag_elem_t elem, int (*cmp)(bag_elem_t, bag_elem_t)) {
    ...
    if (!*root) {
        ...
        if ((avl_remove(&(*root)->left, elem, cmp))) {
            if (HEIGHT((*root)->left) + 1 < HEIGHT((*root)->right))
                avl_rebalance_to_the_left(root);
            else
                avl_update_height(*root);
        ...
        if ((avl_remove(&(*root)->right, elem, cmp))) {
            /* Check if the subtree needs rebalancing; update its height. */
            if (HEIGHT((*root)->right) + 1 < HEIGHT((*root)->left))
                avl_rebalance_to_the_right(root);
            else
